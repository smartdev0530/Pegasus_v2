...
contract Pegasus is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade {
        require(!_upgraded, "Pegasus: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "Pegasus: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __Pegasus_v2_init_unchained(_router);
    }

    function __Pegasus_v2_init_unchained(address _router) internal initializer {
        _name = "Pegasus";
        _symbol = "Pegasus";
        _decimals = 18;

        _tTotal = 5000000 * 10**9 * 10**9;
        ...
        __Pegasus_tiers_init();

        emit Transfer(address(0), _msgSender(), _tTotal);
    }

    function __Pegasus_tiers_init() internal initializer {
        _defaultFees = _addTier(0, 300, 200, 0, 0, address(0), address(0)); // lq fee 3, taxFee 200
    ...
    preventBlacklisted(_account, "Pegasus: Selected account is in blacklist")
    {
        require(_account != address(0), "Pegasus: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner() {
        require(_account != address(0), "Pegasus: Invalid address");
        require(_accountsTier[_account] > 0, "Pegasus: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "Pegasus: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_ecoSystem != address(0), "Pegasus: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_owner != address(0), "Pegasus: Address Zero is not allowed");
    ...
    private
    preventBlacklisted(owner, "Pegasus: Owner address is blacklisted")
    preventBlacklisted(spender, "Pegasus: Spender address is blacklisted")
    {
    ...
    private
    preventBlacklisted(_msgSender(), "Pegasus: Address is blacklisted")
    preventBlacklisted(from, "Pegasus: From address is blacklisted")
    preventBlacklisted(to, "Pegasus: To address is blacklisted")
    isRouter(_msgSender())
    {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, uint256 tierIndex, bool takeFee) private {
        ...
        } else if (_isExcluded[sender] && _isExcluded[recipient]) { //fixed PPC
    ...
    external
    preventBlacklisted(account, "Pegasus: Migrated account is blacklisted")
    override
    {
        require(migration != address(0), "Pegasus: Migration is not started");
        require(_msgSender() == migration, "Pegasus: Not Allowed");
